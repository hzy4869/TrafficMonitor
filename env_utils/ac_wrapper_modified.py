    ...
    def reward_wrapper(self, states, dones) -> float:
        ...
        for aircraft_id, vehicle_info in states.items():
            ...
            if self.latest_veh_pos:
                veh_poses = np.array(list(self.latest_veh_pos.values()))
                best_center = self.compute_cover_center(veh_pos=veh_poses, radius=cover_radius)
                if best_center is not None:
                    self.cluster_point.append(best_center + np.array(aircraft_pos[:2]))
                    m_dist = np.linalg.norm(best_center)
                    if m_dist <= cover_radius + 5:
                        if len(vehicle_info) != 0:
                            add_reward = len(vehicle_info)
                            reward += add_reward
                            log_lines.append(f"[Reward] {aircraft_id}: +{add_reward} (covered {len(vehicle_info)} vehicles)")
                    else:
                        penalty = self.distance_penalty(m_dist - cover_radius, cover_radius, p=25)
                        reward += penalty
                        log_lines.append(f"[Penalty] {aircraft_id}: {penalty:.3f} (too far from cluster, dist={m_dist:.2f})")
                else:
                    log_lines.append(f"[No Cluster] {aircraft_id}: no valid vehicle cluster")
            else:
                log_lines.append(f"[No Vehicle] {aircraft_id}: environment empty")

            # ===== 边界惩罚 =====
            if abs(_y) > (self.y_range - 50):
                reward += -5
                log_lines.append(f"[Boundary Penalty] {aircraft_id}: -5 (near Y boundary)")
            if abs(_x) > (self.x_range - 50):
                reward += -5
                log_lines.append(f"[Boundary Penalty] {aircraft_id}: -5 (near X boundary)")

            if abs(_x) > self.x_range:
                dones = True
                reward += -100
                log_lines.append(f"[Boundary Exceed] {aircraft_id}: -100 (X exceeded)")
                self._write_log(log_lines)  # 写入文件
                return reward, dones
            if abs(_y) > self.y_range:
                dones = True
                reward += -100
                log_lines.append(f"[Boundary Exceed] {aircraft_id}: -100 (Y exceeded)")
                self._write_log(log_lines)  # 写入文件
                return reward, dones
